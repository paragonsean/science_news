...
class ScienceAlertScraper:
    def __init__(self, json_input_file, json_output_file, headers, open_alex_scraper: OpenAlexScraper, article_template: Article):
        ...
        self.output_data = []  # Will hold the scraped or cached articles
        self.cached_articles = {}  # For quick lookup of cached articles by URL
            ...
            async with session.get(final_url, headers=self.headers) as response:
                if response.status == 200:
                    ...
                    if self.article_template.title is not None:
                        title = soup.find('title').text.replace(" : ScienceAlert", "") if soup.find('title') else "N/A"
                        article.title = title
        ...
        while True:
            ...
            try:
                ...
                if entry.url in self.cached_articles:
                    ...
                    if self.article_template.title is not None:
                        entry.title = article_details.title or "N/A"

                    # URL is always updated
                    entry.url = article_details.url

                    if self.article_template.author is not None:
                        entry.author = article_details.author or "N/A"

                    if self.article_template.doi_urls is not None:
                        entry.doi_urls = article_details.doi_urls or ["N/A"]

                    if self.article_template.non_doi_urls is not None:
                        entry.non_doi_urls = article_details.non_doi_urls or ["N/A"]
        ...
        async with aiohttp.ClientSession() as session:
            for key, entry in enumerate(self.article_collection.articles):
                if entry.url:
                    ...
                    if entry.url in self.cached_articles:
                        logger.info(f"Skipping already processed URL: {entry.url}")
                        continue
            ...
            # Create workers
            workers = [asyncio.create_task(self.worker(session, queue)) for _ in range(10)]
...
def main():
    ...
    choice = input("Enter the number of your choice (1 or 3): ").strip()
